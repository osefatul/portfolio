
export const mainProjects = [
    {
        img:'/images/stay-travel.jpg',
        title:"Advanced Travel and Stay Booking System",
        description: "This is an advanced fullstack web application built upon microservices architecture. T&S system is composed of several advanced features including traditional admin rights. The main functionalities for common users are from reserving rooms to booking flights using our own developed API, and an option to check in online which will promptly grant you access to print flight ticket in the PDF format.",
        demoLink:"https://stays-travels-system.netlify.app/",
        githubLink:"https://github.com/osefatul/hotel-reservation-system",
        techStacks:["React","Node","Express","Recharts", "Stripe", "Framer-motion","MongoDB","MUI", "Tailwind" ]

    },

    {
        img:"/images/ticket-system.jpg",
        title:"Advanced Ticketing System",
        description: "Ticketing/Issue tracking system allows organizations to organizations or different departments within an organization to keep track of what needs to get done, who is working on which tasks, what's already been done, and when tasks were (or weren't) completed. ",
        demoLink:"https://advanced-ticketing-system.netlify.app/",
        githubLink:"https://github.com/osefatul/ticket-tracker-system-CRM",
        techStacks:["React","Node","Express","Redis", "Tailwind", "Framer-motion","MongoDB","Nodemailer" ]
    },
    
    {
        img:'/images/amazon-ecommerce.jpg',
        title:"Fullstack Amazon E-commerce System",
        description: "This is a fullstack web application cloning amazon functionalities using MERN stack technologies. We have developed our own API for products, where a user can find different kind of items. A user can also filter items by category and price. A user can store, edit and discard items in cart. Checkout microservice has been added to the application so a user is able to pay for items. A user can also find all items they ordered.",
        demoLink:"https://amazon-ecommerce-system.netlify.app/",
        githubLink:"https://github.com/osefatul/amazon-ecommerce-system/",
        techStacks:["React","Node","Express","MUI", "Stripe","MongoDB" ]

    }

]


